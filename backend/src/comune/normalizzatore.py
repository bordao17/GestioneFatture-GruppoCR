...
# La partita IVA italiana e' sempre 11 cifre; sul documento compare con
# l'etichetta attaccata ("P.IVA 01234567890"), col prefisso IT o spezzata in
# gruppi. Cerchiamo prima 11 cifre contigue, poi ripieghiamo sulle sole cifre.
_PIVA_CONTIGUA = re.compile(r"(?<!\d)(\d{11})(?!\d)")
_PREFISSO_IT = re.compile(r"\bIT\s*(?=\d)", re.IGNORECASE)
...
def normalizza_partita_iva(valore):
    """Le sole 11 cifre della partita IVA, senza etichetta ne' prefisso IT.

    Se non riconosce 11 cifre restituisce comunque le cifre trovate (principio
    del modulo: meglio un dato grezzo che nessun dato). A dire se il valore e'
    utilizzabile come chiave ci pensa partita_iva_valida().
    """
    testo = _pulisci(valore)
    if not testo:
        return ""

    testo = _PREFISSO_IT.sub("", testo)

    trovata = _PIVA_CONTIGUA.search(testo)
    if trovata:
...
def partita_iva_valida(valore):
    """True se il valore e' una partita IVA italiana formalmente corretta.

    Le 11 cifre portano in coda un carattere di controllo (algoritmo di Luhn
    sulle prime 10): e' una regola esatta, quindi sta qui in Python. Serve a
    distinguere una lettura plausibile da una cifra sbagliata dal modello,
    prima di proporla come chiave del fornitore.
    """
    cifre = normalizza_partita_iva(valore)
    if len(cifre) != 11 or not cifre.isdigit():
        return False

    somma = 0
    for posizione, carattere in enumerate(cifre[:10]):
        cifra = int(carattere)
        if posizione % 2 == 0:
            somma += cifra
        else:
    ...
    return (10 - somma % 10) % 10 == int(cifre[10])
...
def normalizza_dati(dati):
    ...
    dati["partita_iva"] = normalizza_partita_iva(dati.get("partita_iva"))
